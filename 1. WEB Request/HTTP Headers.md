As previously seen, HTTP headers are used to pass information between server and client. Some headers are common between both (general headers), and rest are tied to request and response

## Types of headers
There are following types of headers:
...
### General Headers
General headers are common to both request and response. They are contextual and are meant to describe the message then it content
...
### Entity Headers
Define the content (entity) transferred by the message. Can be common to both request and response. Commonly found in responses and POST and PUT requests.
...
### Request headers
Headers that are sent by client during a HTTP transaction. They don't relate to the content of the message. Examples are below
...
### Response Headers
Used in HTTP responses and don't relate to the content. Headers like Age, location and Server can provide more context to the response.
...
### Security Headers
Class of response headers used to define certain rules and policies for browser to follow while accessing the website
