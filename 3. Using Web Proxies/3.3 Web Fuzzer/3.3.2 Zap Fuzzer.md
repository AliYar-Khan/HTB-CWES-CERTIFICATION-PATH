...
![Fuzz Locations interface with HTTP request details on the left and empty fuzz locations list on the right. Options to add, remove, and configure payloads and processors. 'Remove Without Confirmation' is checked.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_fuzzer.jpg)
...
![Fuzz Locations interface with highlighted 'test' in HTTP request. Popup shows location 'Header 31, 35', value 'test', and empty payloads list. 'Remove Without Confirmation' is checked.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_fuzzer_add.jpg)
...
![File Fuzzers interface with 'dirbuster' and 'directory-list-1.0.txt' selected. Payloads preview shows directory names like 'cgi-bin', '.git', '.svn'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_fuzzer_add_payload.jpg)
...
![URL Encode interface with UTF-8 encoding. Current and processed payloads include 'cgi-bin', '.git', '.svn'. 'Lock Scroll' is checked.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_fuzzer_add_processor.jpg)
...
![Options interface for fuzzing. Retries on IO error: 3. Limit max errors: 1000. Depth First strategy selected. 20 concurrent threads. No delay in fuzzing. Follow Redirects unchecked.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_fuzzer_options.png)
...
![Table showing task IDs, message types, HTTP codes, reasons, round-trip times, response sizes, states, and payloads. Example: Task ID 908, 200 OK, 109 ms, 246 bytes, state 'skills'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_fuzzer_attack.jpg)

As we can see, we got one hit with code `200` with the `skills` payload, meaning that the `/skills/` directory exists on the server and is accessible. We can click on the request in the results window to view its details: ![Request and response details. Request: HTTP GET to /skills/. Response: 200 OK, includes Set-Cookie header, HTML content with 'Welcome' title.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_fuzzer_dir.jpg)
