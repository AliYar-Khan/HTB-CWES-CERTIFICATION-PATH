...
Both Burp and ZAP have extension capabilities, such that the community of Burp users can develop extensions for Burp for everyone to use. Such extensions can perform specific actions on any captured requests, for example, or add new features, like decoding and beautifying code. Burp allows extensibility through its `Extensions` feature and its [BApp Store](https://portswigger.net/bappstore), while ZAP has its [ZAP Marketplace](https://www.zaproxy.org/addons/) to install new plugins.

---

## BApp Store

To find all available extensions, we can click on the `Extensions` tab within Burp and select the `BApp Store` sub-tab. Once we do this, we will see a host of extensions. We can sort them by `Popularity` so that we know which ones users are finding most useful:

![BApp Store showing Burp extensions. Active Scan++ highlighted, last updated 25 Mar 2021. Features include host header attacks, edge side includes, XML input handling.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/burp_bapp_store.png)
...
![BApp Store showing Burp extensions. Decoder Improved installed, last updated 19 Feb 2021. Features include encoding, decoding, hashing, tabs, Unicode support, and hex editor.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/burp_extension.png)
...
![Decoder Improved interface showing 'HTB Academy' text, hex view, and options for hashing with MD5 and encoding as plain text.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/burp_extension_decoder_improved.jpg)
...
![Toolbar with 'Standard Mode' selected, 'Sites' section, and 'Manage Add-ons' button highlighted.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_marketplace_button.jpg)

In this tab, we can see the different available add-ons for ZAP. Some add-ons may be in their `Release` build, meaning that they should be stable to be used, while others are in their `Beta/Alpha` builds, which means that they may experience some issues in their use. Let's try installing the FuzzDB Files and FuzzDB Offensive add-ons, which add new wordlists to be used in ZAP's fuzzer:  ![Marketplace tab showing add-ons. Selected: FuzzDB Files and FuzzDB Offensive, both in Release status. Options to install or get more info.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_fuzzdb_install.png)
...
![File Fuzzers type selected. Files listed with 'command_execution-unix.txt' checked. Payloads preview shows command execution examples.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_fuzzdb_cmd_exec.png)
...
![Fuzzer progress at 62% on http://139.59.6.56:31133/ping. Messages sent: 34, Errors: 8. Task IDs with status 200 OK, showing payloads like ';id' and '/usr/bin/id'.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_fuzzer_cmd_inj.jpg)
