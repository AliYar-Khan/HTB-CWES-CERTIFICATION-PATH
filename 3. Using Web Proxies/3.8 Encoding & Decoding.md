...
It is essential to ensure that our request data is URL-encoded and our request headers are correctly set. Otherwise we may get error responses from server.
...
To URL-encode text in Burp Repeater, we can select the text and right-click on it, then select (`Convert Selection>URL>URL-encode key characters`), or by selecting the text and clicking `CTRL+U`. On the other hand, ZAP should automatically URL-encode all of our request data in the background before sending the request, though we may not see that explicitly.
...
![Decoder tab showing encoded string and its decoded JSON output with options for text, hex, and smart decode.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/burp_b64_decode.png)
...
![Request and response details with encoded IP in request, HTML response, and Inspector showing URL and Base64 decoding.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/burp_inspector.png)

In ZAP, we can use the `Encoder/Decoder/Hash` tool, which will automatically decode strings using various decoders in the `Decode` tab:![Text encoding/decoding interface showing Base64 encoded string and its decoded JSON output.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_b64_decode.png)
...
![Decoder tab showing JSON with admin access and its Base64 encoded string, with options for text, hex, and smart decode.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/burp_b64_encode.png)

![Text encoding interface showing JSON with admin access and its Base64 encoded output.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/110/zap_b64_encode.png)
