...
```shellsession
aliyark145@htb[/htb]$ curl -I inlanefreight.com
```
...
```shellsession
aliyark145@htb[/htb]$ curl -I https://inlanefreight.com 
HTTP/1.1 301 Moved Permanently 
...
```shellsession
aliyark145@htb[/htb]$ curl -I https://www.inlanefreight.com 
HTTP/1.1 200 OK 
...
Link: <https://www.inlanefreight.com/>; 
rel=shortlink 
Content-Type: text/html; 
charset=UTF-8
```
...
```shellsession
aliyark145@htb[/htb]$ pip3 install git+https://github.com/EnableSecurity/wafw00f
```
...
```shellsession
aliyark145@htb[/htb]$ sudo apt update && sudo apt install -y perl aliyark145@htb[/htb]$ git clone https://github.com/sullo/nikto aliyark145@htb[/htb]$ cd nikto/program aliyark145@htb[/htb]$ chmod +x ./nikto.pl
```

To scan `inlanefreight.com` using `Nikto`, only running the fingerprinting modules, execute the following command:

```shellsession
aliyark145@htb[/htb]$ nikto -h inlanefreight.com -Tuning b
```
