...
```shellsession
aliyark145@htb[/htb]$ pip3 install scrapy
```
...
```shellsession
aliyark145@htb[/htb]$ wget -O ReconSpider.zip https://academy.hackthebox.com/storage/modules/144/ReconSpider.v1.2.zip aliyark145@htb[/htb]$ unzip ReconSpider.zip
```

With the files extracted, you can run `ReconSpider.py` using the following command:

```shellsession
aliyark145@htb[/htb]$ python3 ReconSpider.py http://inlanefreight.com`
```
