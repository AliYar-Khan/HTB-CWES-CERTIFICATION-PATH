...
```shellsession
aliyark145@htb[/htb]$ gobuster vhost -u http://<target_IP_address> -w <wordlist_file> --append-domain
```
...
```shellsession
aliyark145@htb[/htb]$ gobuster vhost -u http://inlanefreight.htb:81 -w /usr/share/seclists/Discovery/DNS/subdomains-top1million-110000.txt --append-domain 
...
**Assessment**
create a custom list with the prefixes web, vm, br, a and su by using command below:
```
grep -Eh '^(web|vm|br|a|su)' /usr/share/seclists/Discovery/DNS/*
```

Sol 1: brute forcing with go buster using the commands reveals the web17661.inlanefreight.htb

```
gobuster vhost -u http://<target>:<port> -w ./custom_list.txt --append-domain
```

Sol 2: vm5.inlanefreight.htb

Sol 3: browse.inlanefreight.htb

Sol 4: admin.inlanefreight.htb

Sol 5: support.inlanefreight.htb
