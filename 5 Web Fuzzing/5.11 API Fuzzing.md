...
```shellsession
aliyark145@htb[/htb]$ git clone https://github.com/PandaSt0rm/webfuzz_api.git aliyark145@htb[/htb]$ cd webfuzz_api aliyark145@htb[/htb]$ pip3 install -r requirements.txt
```

Then, run the fuzzer using the spawned target IP and PORT

```shellsession
aliyark145@htb[/htb]$ python3 api_fuzzer.py http://IP:PORT 
[-] Invalid endpoint: http://localhost:8000/~webmaster (Status code: 404) 
[-] Invalid endpoint: http://localhost:8000/~www (Status code: 404) 
Fuzzing completed. Total requests: 4730 Failed requests: 0 Retries: 0 Status code counts: 404: 4727 200: 2 405: 1 Found valid endpoints: - http://localhost:8000/cz... - http://localhost:8000/docs Unusual status codes: 405: http://localhost:8000/items
```
...
```shellsession
aliyark145@htb[/htb]$ curl http://localhost:8000/cz... 
{"flag":"<snip>"}
```
