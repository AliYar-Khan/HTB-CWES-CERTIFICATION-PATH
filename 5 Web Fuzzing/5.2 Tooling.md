...
```shellsession
aliyark145@htb[/htb]$ sudo apt update
```  

2. Use the following command to install Go:

```shellsession
aliyark145@htb[/htb]$ sudo apt install -y go-lang
```

3. Use the following command to install Python:

```shellsession
aliyark145@htb[/htb]$ sudo apt install python3 python3-pip
```

4. Use the following command to install and configure pipx:

```shellsession
aliyark145@htb[/htb]$ sudo apt install pipx 
aliyark145@htb[/htb]$ pipx ensurepath 
aliyark145@htb[/htb]$ sudo pipx ensurepath --global`
```

5. To ensure that Go and Python are installed correctly, you can check their versions:
```shellsession
aliyark145@htb[/htb]$ go version 
aliyark145@htb[/htb]$ python3 --version
```
...
```shellsession
aliyark145@htb[/htb]$ go install github.com/ffuf/ffuf/v2@latest
```
...
```shellsession
aliyark145@htb[/htb]$ go install github.com/OJ/gobuster/v3@latest
...
```shellsession
aliyark145@htb[/htb]$ curl -sL https://raw.githubusercontent.com/epi052/feroxbuster/main/install-nix.sh | sudo bash -s $HOME/.local/bin
```
