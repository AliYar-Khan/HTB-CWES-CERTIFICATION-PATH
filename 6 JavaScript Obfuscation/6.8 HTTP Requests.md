...
```shellsession
aliyark145@htb[/htb]$ curl http://SERVER_IP:PORT/ 
...
```shellsession
aliyark145@htb[/htb]$ curl -s http://SERVER_IP:PORT/ -X POST
```
...
```shellsession
aliyark145@htb[/htb]$ curl -s http://SERVER_IP:PORT/ -X POST -d "param1=sample"
```
