...
---
## Reflected XSS
It is vulnerability when a user input is sent to server and then return back to us without being filtered or sanitized. 

**Examples**:
error messages or confirmation messages 
However, as these are usually temporary messages, once we move from the page, they would not execute again, and hence they are `Non-Persistent`.
...
![To-Do List interface with a text input labeled 'Your Task' and an 'Add' button. Error message: 'Task 'test' could not be added.'](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_reflected_1.jpg)
...
![IP address 139.59.166.56:31323 with 'OK' button.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_stored_xss_alert.jpg)
...
![Network tab showing HTTP requests: 200 status for localhost index.php, bootstrap.min.js, jquery.min.js; 404 status for localhost favicon.ico.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_reflected_network.jpg)
...
![IP address 139.59.166.56:31323 with 'OK' button.|697](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_stored_xss_alert.jpg)


**Assessment**

Sending the GET request with payload shows the cookie in popup

```html
<div></div><ul class="list-unstyled" id="todo"><div style="padding-left:25px">Task '<script>alert(document.cookie)s</script>' could not be added.</div></ul>
```
