...
![Online Image Viewer with Hack The Box logo.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_phishing_image_viewer.jpg)
...
![Online Image Viewer with Image URL input and login fields.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_phishing_injected_login_form.jpg)
...
To find the `id` of the HTML element we want to remove, we can open the `Page Inspector Picker` by clicking `CTRL+SHIFT+C` and then clicking on the element we need: ![Page Inspector Picker](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_page_inspector_picker.jpg)
...
![Online Image Viewer with Image URL input and login fields for Username and Password.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_phishing_injected_login_form_2.jpg)
...
![Online Image Viewer with Username and Password login fields.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_phishing_injected_login_form_3.jpg)
...
```shellsession
aliyark145@htb[/htb]$ sudo nc -lvnp 80 listening on [any] 80 ...
```
...
```shellsession
aliyark145@htb[/htb]$ mkdir /tmp/tmpserver aliyark145@htb[/htb]$ cd /tmp/tmpserver aliyark145@htb[/htb]$ vi index.php #at this step we wrote our index.php file aliyark145@htb[/htb]$ sudo php -S 0.0.0.0:80 PHP 7.4.15 Development Server (http://0.0.0.0:80) started
```
...
![Online Image Viewer with Image URL input.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_image_viewer.jpg)

If we check the `creds.txt` file in our Pwnbox, we see that we did get the login credentials:

```shellsession
aliyark145@htb[/htb]$ cat creds.txt Username: test | Password: test
```
...
lab achievement https://labs.hackthebox.com/achievement/machine/482394/441