...
Consider an example for authentication. Front-end passes the username and password and final query will look like below 

```sql
SELECT * FROM logins WHERE username='admin' AND password = 'p@ssw0rd';
```

If credentials are correct, then condition evaluates to true, else false.
...
If we insert single quote as username, the query becomes:
...
![SQL query logic diagram: SELECT * FROM logins WHERE username='admin' OR '1'='1' AND password='something'. Logic flow shows OR condition as True, making the entire statement True despite the False password condition](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/33/or_inject_diagram.png)
...
Next, the OR operator is evaluated:
	...
	.The '1'='1' condition is irrelevant in this context because it doesn't
	affect the outcome of the AND condition.
...
![SQL query logic diagram: SELECT * FROM logins WHERE username='notAdmin' OR '1'='1' AND password='something'. Logic flow shows OR condition as False, making the entire statement False](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/33/notadmin_diagram_1.png)
...
![Admin panel showing an SQL query execution: SELECT * FROM logins WHERE username='notAdmin' OR '1'='1' AND password='something' OR '1'='1'; with a message: Login successful as user: admin](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/33/password_or_injection.png)
...
![Admin panel showing an SQL query execution: SELECT * FROM logins WHERE username='' OR '1'='1' AND password='something' OR '1'='1'; with a message: Login successful as user: admin](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/33/basic_auth_bypass.png)
...
bypassed the login successfully