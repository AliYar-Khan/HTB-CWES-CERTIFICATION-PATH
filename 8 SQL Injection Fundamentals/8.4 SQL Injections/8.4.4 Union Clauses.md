...
mysql> SELECT city FROM ports UNION SELECT * FROM ships; ERROR 1222 (21000): The used SELECT statements have a different number of columns
```
...
Query returns 663 rows

```sql
select dept_no, dept_name, 3, 4, 5, 6 from departments union select * from employees
```

3,4,5,6 were added because departments has less no of columns than employees