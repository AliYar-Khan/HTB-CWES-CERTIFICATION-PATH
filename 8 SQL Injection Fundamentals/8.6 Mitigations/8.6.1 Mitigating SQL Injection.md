...
$result = mysqli_query($conn, $query);  $row = mysqli_fetch_array($result); <SNIP>
```
...
```php
<?php if (isset($_GET["port_code"])) {     
$q = "Select * from ports where port_code ilike '%" . $_GET["port_code"] . "%'";    $result = pg_query($conn,$q);         
	if (!$result)    {        
...
```shellsession
aliyark145@htb[/htb]$ mysql -u reader -p 
...
mysqli_stmt_bind_param($stmt, 'ss', $username, $password);  mysqli_stmt_execute($stmt);  
